document.addEventListener('DOMContentLoaded', function (event) {
    var settings = {
        ...
        filterHtml: function (html) {
            return html.replace(/<div[^>]+class\s*=\s*['"]demo-topbar['"][\s\S]*?<\/\s*div\s*>/ig, '').replace(/<(a|button)[^>]+data-codepen[^>]*>[\s\S]*?<\/\s*\1\s*>/ig, '');
        }
    };
    ...
    document.querySelectorAll('a[data-codepen], button[data-codepen]').forEach(function (el) {
        el.addEventListener('click', function () {
            ...
            openEditor(url ? normalizeUrl(url, window.location.href) : window.location.href);
        });
    });
    ...
    function openEditor(url) {
        ...
        getUrlContent(url, function (content) {
            var codepenData = buildCodepenData(content, url // all relative refs within the content will be relative to this baseUrl
            );

            newWindow.document.open();
            newWindow.document.write('<html><body>' + '<form action="https://codepen.io/pen/define" method="POST">' + '<input type="hidden" name="data" value="' + escapeHtml(JSON.stringify(codepenData)) + '" />' + '</form>' + '<script>document.forms[0].submit()</script>' + '</body></html>');
    ...
    function buildCodepenData(html, baseUrl) {
        ...
        if ((match = BODY_RE.exec(html))) {
            bodyHtml = normalizeCode(absolutizeHtmlRefs(settings.filterHtml(match[2], baseUrl), baseUrl));
        }
        ...
        while (match = LINK_RE.exec(html)) {
            if (parseAttribute(match[1], 'rel') === 'stylesheet' && parseAttribute(match[1], 'media') !== 'print' && // exclude print-only stylesheets
        ...
        return {
            ...
            editors: ((!settings.collapseHtml && bodyHtml) ? '1' : '0') + ((!settings.collapseCss && inlineCssBlocks.length) ? '1' : '0') + ((!settings.collapseJs && inlineJsBlocks.length) ? '1' : '0')
        };
    }
    ...
    function computeCommonIndent(s) {
        var RE = /^[\t ]+/mg;
        // specifically test for spaces/tabs to avoid matching newlines
        var indents = [];
        var match;
    ...
    function normalizeUrl(href, baseUrl) {
        var HOST_RE = /^(\w+:\/\/([^\/]+))(.*)$/;
        // also matches the protocol like https://
        var baseHostMatch;
        var parts;
        var stack;
        var i;
